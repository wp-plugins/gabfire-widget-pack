...
class gabfire_tweets extends WP_Widget {
	...
	function update($new_instance, $old_instance) {
		...
		$instance['tweets_base'] = strip_tags($new_instance['tweets_base']);
		$instance['tweets_of'] = strip_tags($new_instance['tweets_of']);
		$instance['profile_photo'] = strip_tags($new_instance['profile_photo']);
		$instance['tweets_nr'] = (int)$new_instance['tweets_nr'];
		return $new_instance;
	}
			...
			if ($tweets_base == 'hashtag'){		
				...
				foreach($reply['statuses'] as $message) {
					...
					if ($options['profile_photo'] == 'display') {
						$out .= '<img class="alignright" src="'.$message['user']['profile_image_url_https'].'" />';
					}
